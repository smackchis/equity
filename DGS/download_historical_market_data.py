...
for index, row in df.iterrows():
    ...
    for md_date, row in stock_hist_price.iterrows():
        ...
        if (str(symbol) + str(md_date)) in df_check_db_presence.values: # makes that no duplicate data in inserted into DB
            ...
            insert_query = insert_query + f"insert into dbo.historical_market_data ([date], [symbol], [series], [prev_close], [open], [High], [Low], [last], [close], [vwap], [volume], [turnover], [Deliverable_Volume], [percent_Deliverble], [Trades]) values ('{md_date}','{Symbol}','{Series}', {Prev_Close}, {Open}, {High}, {Low}, {Last}, {Close}, {VWAP}, {Volume}, {Turnover}, {Deliverable_Volume}, {percent_Deliverble}, {Trades});"
